  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white selection:bg-orange-500/30">
      ...
      <section className="relative pt-32 pb-24 overflow-hidden min-h-[80vh] flex items-center justify-center">
        <div className="absolute inset-0 bg-cover bg-center bg-no-repeat opacity-60" style={{ backgroundImage: `url(${penchHero})` }} />
        <div className="absolute inset-0 bg-gradient-to-b from-black/70 via-black/40 to-[#020617]" />
        
        <div className="container mx-auto px-4 relative z-10 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <span className="inline-block px-4 py-1 rounded-full bg-orange-500/20 border border-orange-500/30 text-orange-400 text-xs font-bold uppercase tracking-widest mb-6">
              India's First Dark Sky Park
            </span>
            <h1 className="text-5xl md:text-8xl font-display font-bold mb-6 tracking-tighter uppercase leading-none">
              Pench Astro <br/><span className="text-orange-500">Wildlife</span> Tour
            </h1>
            <p className="text-xl md:text-2xl text-white/80 font-light max-w-3xl mx-auto italic mb-10">
      ...
      <section className="py-24 container mx-auto px-4">
        <div className="grid md:grid-cols-4 gap-8">
          {[
            ...
            { icon: <Binoculars className="w-8 h-8 text-green-400" />, title: "Tiger Safari", desc: "Thrilling morning and afternoon safaris in the land of Mowgli." },
            ...
            <motion.div key={i} whileHover={{ y: -10 }} className="p-8 bg-white/5 border border-white/10 rounded-[2.5rem] hover:bg-white/[0.08] transition-all">
              <div className="mb-6">{item.icon}</div>
              <h3 className="text-2xl font-display font-bold mb-4">{item.title}</h3>
              <p className="text-white/50 leading-relaxed text-left">{item.desc}</p>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      <section className="py-24 bg-white/[0.02]">
        <div className="container mx-auto px-4 grid lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-8">
            <h2 className="text-4xl md:text-6xl font-display font-bold uppercase tracking-tighter">The Jungle Book <br/><span className="text-orange-500">Legacy</span></h2>
            ...
            <div className="flex gap-8 pt-4 justify-start">
              <div className="text-center">
                ...
                <span className="text-xs uppercase tracking-widest text-white/40">Night Sessions</span>
              </div>
              <div className="text-center">
                <span className="block text-4xl font-bold text-orange-500">2</span>
                <span className="text-xs uppercase tracking-widest text-white/40">Jungle Safaris</span>
              </div>
              <div className="text-center">
                <span className="block text-4xl font-bold text-orange-500">3</span>
                <span className="text-xs uppercase tracking-widest text-white/40">Days Adventure</span>
              </div>
            </div>
          </div>
      ...
      <section className="py-24 container mx-auto px-4 max-w-5xl">
        <h2 className="text-4xl font-display font-bold text-center mb-16 uppercase tracking-widest">Tour Itinerary</h2>
        <div className="space-y-12">
          {itinerary.map((day, i) => (
            <div key={i} className="relative pl-12 border-l-2 border-orange-500/30">
              ...
              <h3 className="text-3xl font-display font-bold text-orange-400 mb-8 uppercase text-left">{day.day}</h3>
              <div className="grid gap-6">
                {day.events.map((event, j) => (
                  <div key={j} className="flex flex-col md:flex-row md:items-center gap-4 p-6 bg-white/5 border border-white/5 rounded-3xl">
                    <span className="font-mono text-cyan-400 w-48 shrink-0 text-left">{event.time}</span>
                    <span className="text-white/80 font-medium text-lg text-left">{event.activity}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* Package Details */}
      <section className="py-24 container mx-auto px-4">
        <div className="bg-gradient-to-br from-orange-600/20 to-purple-600/20 border border-white/10 rounded-[3rem] p-12 overflow-hidden relative">
          <div className="absolute top-0 right-0 w-64 h-64 bg-orange-500/10 blur-[100px] -z-10" />
          <div className="grid lg:grid-cols-2 gap-16">
            <div>
              <h2 className="text-4xl font-display font-bold mb-8 uppercase text-left">What's Included</h2>
              <ul className="space-y-4">
                {[
                  ...
                  <li key={i} className="flex items-center gap-4 text-white/70 text-left">
                    <div className="p-2 bg-orange-500/20 rounded-lg text-orange-400 shrink-0">{item.icon}</div>
                    {item.text}
                  </li>
                ))}
              </ul>
            </div>
            <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-10">
              ...
              <div className="space-y-6 mb-10">
                <div className="flex justify-between items-center pb-6 border-b border-white/10">
                  ...
                  <span className="text-3xl font-display font-bold">₹24,900</span>
                </div>
                <div className="flex justify-between items-center pb-6 border-b border-white/10">
              ...
              <button className="w-full py-5 bg-white text-black font-bold rounded-2xl hover:scale-[1.02] transition-transform uppercase tracking-widest">
                Enquire for Booking
              </button>
